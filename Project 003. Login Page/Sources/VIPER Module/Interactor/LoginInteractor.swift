...
// 3. Презентер сообщает Интерактору о том, что Интерактор должен создать аккаунт
...
// 4.2. Интерактор возвращает сообщение о том, что аккаунт создан (или об ошибке) + дополнительные данные
...
final class LoginInteractor: CreateNewAccountInteractorInput {
    ...
    // 4.1. Интерактор идет в Модель и берет оттуда данные
    func createNewAccountWith(login: String, password: String) {
        let _ = LoginEntity(login: login, password: password)
        // Прописывается дополнительная реализация того, что еще должно произойти при показе аккаунта пользователя
